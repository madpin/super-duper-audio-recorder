...
export class AudioRecorderSettingTab extends PluginSettingTab {
    ...
    async display(): Promise<void> {
        ...
        new Setting(containerEl)
            .setName('Safety & Duration')
            .setDesc('Settings for data safety and recording duration limits.')
            .setHeading();

        new Setting(containerEl)
            .setName('Auto-save interval')
            .setDesc('Automatically save a backup of the recording every X minutes. Set to 0 to disable.')
            .addText(text => text
                .setPlaceholder('Minutes')
                .setValue(this.plugin.settings.autoSaveInterval.toString())
                .onChange(async (value) => {
                    this.plugin.settings.autoSaveInterval = parseFloat(value) || 0;
                    await this.plugin.saveSettings();
                }));

        new Setting(containerEl)
            .setName('Split file duration')
            .setDesc('Automatically start a new file every X minutes. Set to 0 to disable.')
            .addText(text => text
                .setPlaceholder('Minutes')
                .setValue(this.plugin.settings.splitFileDuration.toString())
                .onChange(async (value) => {
                    this.plugin.settings.splitFileDuration = parseFloat(value) || 0;
                    await this.plugin.saveSettings();
                }));
